...
<!-- TODO in this section and/or the full Prolly construction, specify that we chunk in units of key-value pairs and don't wanna break stuff up -->
...
Say we want a state-based CRDT that implements a key-value store with some ordering on the keys (e.g. the keys are integers).
So we can represent the state as a sequence of key-value pairs $(k_i, v_i)$. Now:

1. Run a content-defined chunker (a rolling hash and judgment function) on the input key-value pairs, checking for boundaries after the end of each pair (so we don't break apart the keys or values).
2. Then for each block,
   1. put it in our content-addressable storage,
   2. take the hash of that block $h_j$,
   3. and pair that hash with the smallest key $k_j$ that was held in that block.
3. Now we have a new sequence of key-value pairs $(k_j, h_j)$ (though the values are hashes). Recurse on this new sequence until there is only one key-hash pair remaining. This is the root of the Prolly Tree.
...
<!--
TODO do we annotate with keys, or is that part of the data? I can see two worlds.
One says that after the first round of hashing, we simply concatenate those hashes and do the next round of hashing on just the hashes. Then we have no search tree info, so we have to annotate with where to find what child. And that metadata doesn't factor into any further hashing.
The other says that we treat each parent node as a key-value thing, where the key is the e.g. smallest key in its child blocks and the value is the hash pointer to the next node down. Then we don't need to do any extra hashing, and it's a bit of a nicer schema because we always are dealing with key-value pairs. (But at the leaves it's key-realvalue, instead of key-hashvalue.)
I like the second one the most. And it seems most accurate to the original Noms diagrams. https://github.com/attic-labs/noms/blob/master/doc/intro.md#prolly-trees-probabilistic-b-trees
TODO update description with this new knowledge
--> 

To compare states between nodes, X.
