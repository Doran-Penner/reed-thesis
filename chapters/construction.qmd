...
<!-- Chunking is commonly used for storing large amounts of data, since there are often duplicated blocks between large files. Instead of storing the large file directly, it stores a series of pointers to chunks. Then if any chunks are shared between files, there's no duplicated data. -->
Generally, _chunking_ is the process of breaking up a large sequence of data into smaller blocks, or chunks.
The simplest form of chunking is fixed-length: draw a dividing line every $k$ bytes and those are the blocks. (This is what we did initially with the Merkle tree.) This is a useful deduplication system, but as shown above it doesn't take full advantage of data similarity.

We can't fix the chunk-shift problem entirely, but we can make it less likely to happen with _content-defined chunking_.
...
TODO give better explanation of rolling hash, use academic references, etc. I should just skip to a rolling hash and not talk about this fake intermediary.

How can this be done? The common solution is with a _rolling hash_.

TODO I'm tired and unenthusiastic about explaining the details of rolling hashes. I'm gonna move on for now, but will get back to this.

- explain CDC simply: tries to fix the boundary-shift problem (in many areas besides distsys) while still deduplicating things and being deterministic.
- maybe go into detail. If yes...
  - give explanation of simple gear hash (judgment function)
  - say "future work fixes problems like normalization, but that's not our problem"
- otherwise can black-box it and say that we have neat tools to do this.
  - Don't love that approach but it might be best, and I can add references to papers. (Note that the one blog post empirically didn't have big boundary-shift issues, though that's limited.)
- recap and say "look we now have a better merkle tree for state-based crdts"

***

The hash of a chunk will always be the same, since hash functions are deterministic. However, it is pseudorandom and doesn't follow any nice pattern like "every $k$ bytes we draw a line;" this actually is beneficial in this case.
