...
- recap and say "look we now have a fancier merkle tree for state-based crdts"
...
What we did initially with the Merkle tree is fixed-length chunking: draw a dividing line every $k$ bytes and those are the blocks. This is a useful deduplication system, but as shown above it doesn't take full advantage of data similarity. _Content-defined chunking_ (CDC) attempts to fix this boundary-shift problem by determining chunk boundaries (when to draw a line between chunks) based on the actual contents being chunked. It doesn't fix the duplication problem entirely, but good CDC is reasonably efficient and reduces the problem when new data is inserted.

::: {#def-rolling-hash}

### Rolling hash

A _rolling hash_ computes a random-seeming (but deterministic) value of a sliding window of data. TODO improve that definition.

:::

Notably, these are not collision-resistant hash functions that we covered earlier --- but they're not made to have low chance of collision.
Instead, rolling hashes provide an efficient way to do CDC by TODO HOW DO I SAY WHAT IT DOES.
TODO explain how they work and give examples. Maybe drop the definition environment and instead just have a big paragraph explaining how they work in theory.

A very simple rolling hash is the Gear hash [@ddelta]. TODO give the recursive definition and talk about its properties.
