...
<!-- TODO this is a place to use sequence notation -->
...
<!-- TODO I wanna give formal math-y definition of a chunker, and maybe use the same notation for ranges. How should I notate these? Normal sequence parentheses, big comma/kappa, dotdotdot, function from $\mathbb{N}$, square-bracket set builder, CS array notation... so many ways. Ugh. Also "chunk" or "block" for the created thing? -->

TODO rework this section based on meeting discussion. Instead I will give a high-level idea of what content-defined chunking gives us, then use gear hashing as a simple-but-possibly-flawed example, then say "assume we have access to a good CDC system" or something like that.
Alsoooo I need to decide on notation for chunking output.
Just gotta pick something!
Easiest is maybe... let $S$ be the set of all possible finite input arrays/sequences/whatever (e.g. array of 32-bit integers). Then our content-defined chunker C is defined as
\begin{align*}
C ~:\quad S \quad &\to 2^{\mathbb{N}^+} \\
(s_j)_{j=1}^n &\mapsto I
\end{align*}
where the elements of $I$ are indices into the input and $|I| \le n$.

***

<!-- TODO remember Marcus' office discussion. Might be good to work through an example. And I am more confident in my strategy of "here's a simple setup, it gets fancier but just assume that we have some thing which will do chunking." -->
...
<!-- TODO I don't have the license for this or some other images -->
...
<!-- TODO say more. I feel like there's a bit lacking, but let's write chapter 3 and then come back to this. -->

## Comparison of similar structures

This "prolly tree" was originally created in @noms. But there's a lot of other similar stuff!

<!-- TODO perhaps say that this is what we already had before the search tree section -->
If you already have an ordering on the data and don't need searchability, you can just do a merkle tree with content-defined chunking. This is what @content-defined-merkle-tree does for very large files with lots of data similarity.
<!-- TODO should I talk more about this? Or say that it's used by huggingface? -->

There's another thing called a Merkle Search Tree [@mst] which is constructed a bit differently but gets to the same end result of a history-independent search tree.
<!-- TODO do I wanna say more? See the large commented-out block. -->

And there are a number of other parallel inventions of the same structure --- see BLOG POST (CITE, and follow-up?) for more details.
We choose to use the Prolly Tree because TODO: fewer edge cases, we want searchability, anything else? At some point you just gotta pick something.

<!-- We chose Dolt version of the tree --- it has fewer weird questions like “what if there’s a big hash level gap” or whatever. But MST is also good! It's used in e.g. Bluesky (CITE). -->
