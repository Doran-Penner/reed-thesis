...
TODO where do I define all the terminology like "node?"
...
<!-- do I actually want this definition style, or should I keep it in prose? -->
::: {#def-distsys}
...
TODO some explanation for why we care about p2p. Resiliency, independence, better load/speed, and so on. And sometimes you have to deal with non-hierarchical interactions so it's necessary.

A _peer-to-peer_ system addresses these problems.
For the first conference on peer-to-peer networking, peer-to-peer is defined as when
...
This is a complex definition and already shows the nuances of peer-to-peer systems. Most of the time, peer-to-peer is defined in contrast to a client-server architecture --- instead of defining nodes as clients that request data or servers that respond to requests, all machines are equal in some sense and can operate as both clients and servers. A simple example of peer-to-peer program is BitTorrent [@sam-thesis], which is used for file sharing: TODO explain the benefits (via Sam's intro) of p2p file sharing.
<!-- But there are many ways to define peer-to-peer systems. -->
